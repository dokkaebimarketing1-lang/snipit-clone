...
export async function uploadAds(formData: FormData) {
  ...
  for (const file of files.slice(0, 20)) {
    ...
    if (uploadError) { errors.push({ fileName: file.name, error: uploadError.message }); continue; }

    const { data: urlData } = supabase.storage.from("ad-uploads").getPublicUrl(filePath);
    ...
    if (insertError || !savedAd) { errors.push({ fileName: file.name, error: insertError?.message || "Insert failed" }); continue; }
    results.push({ id: savedAd.id, imageUrl: urlData.publicUrl, fileName: file.name });
  }

  revalidatePath("/board");
  return { uploaded: results.length, failed: errors.length, results, errors: errors.length > 0 ? errors : undefined };
}
